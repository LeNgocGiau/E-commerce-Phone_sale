...
var deleteModal = document.getElementById('delete-product');
...
function addProduct() {
    ...
    if (checkSameID(productID)) {
        ...
        // alert(`Thêm sản phầm ${productID} thành công!`);
...
function showEditModal(name) {
    ...
    productInfo.oldPrice = covertPriceToNumber(productInfo.oldPrice);
    productInfo.currentPrice = covertPriceToNumber(productInfo.currentPrice);
    ...
    productOldPrice.value = productInfo.oldPrice;
    productCurrentPrice.value = productInfo.currentPrice;
}
...
function showDeleteModal(name) {
    ...
    document.querySelector('.delete-form__question').innerHTML = `Bạn có muốn xóa sản phẩm ${productInfo.name} không ?`;
}
